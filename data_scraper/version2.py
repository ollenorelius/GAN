...
def get_links(query_string, num_images):
    ...
    for i in range(0,num_images,100):
        ...
        json_string = urllib.request.urlopen(request).read().decode("utf8")
...
def get_images(links,directory,pre):
    ...
    for i in range(len(links)):
        try:
            print("Saving: %s" % links[i])
            urllib.request.urlretrieve(links[i], "./"+directory+"/"+str(pre)+"_"+str(i)+".jpg")
        except:
            pass
...
def search_images(base,terms,num_images):
    for y in range(len(base)):
        for x in range(len(terms)):
            ...
            get_images(all_links,base[y],y)

if __name__ == '__main__':
    terms = [""]
    base = ["dog", "car", "human", "graph"]
